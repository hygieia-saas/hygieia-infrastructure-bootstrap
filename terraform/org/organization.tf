...
resource "aws_organizations_account" "infra_homepage_prod" {
  provider = aws.root

  name  = local.account_info_infra_homepage_prod.name
  email = local.account_info_infra_homepage_prod.email
...
resource "aws_organizations_account" "infra_webapp_preprod" {
  provider = aws.root

  name  = local.account_info_infra_webapp_preprod.name
  email = local.account_info_infra_webapp_preprod.email
  parent_id = aws_organizations_organizational_unit.infra.id
}
